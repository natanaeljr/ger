...
pub fn show(config: &mut CliConfig, change: &ChangeInfo) -> Result<(), failure::Error> {
  ...
  writeln!(stdout, "Updated:     {}", util::format_long_datetime(&change.updated.0))?;
  ...
  if let Some(current_commit) = current_commit {
    ...
    if let Some(committer) = &current_commit.committer {
      writeln!(stdout, "Committer:   {} <{}>", committer.name, committer.email)?;
    }
  }
    ...
    if !current_files.is_empty() {
      ...
      for file in current_files {
        match &file.1.status {
          ...
          FileStatus::Added => stdout.set_color(ColorSpec::new().set_fg(Some(Color::Green)).set_intense(true))?,
          FileStatus::Deleted => stdout.set_color(ColorSpec::new().set_fg(Some(Color::Red)))?,
          FileStatus::Renamed => stdout.set_color(ColorSpec::new().set_fg(Some(Color::Yellow)))?,
          FileStatus::Copied => stdout.set_color(ColorSpec::new().set_fg(Some(Color::Magenta)).set_intense(true))?,
          FileStatus::Rewritten => stdout.set_color(ColorSpec::new().set_fg(Some(Color::White)).set_intense(true))?,
        }
...
pub fn show_messages(config: &mut CliConfig, messages: &Vec<ChangeMessageInfo>) -> Result<(), failure::Error> {
  let mut stdout = config.stdout.lock();
  ...
  for message in messages {
    ...
    stdout.set_color(ColorSpec::new().set_fg(Some(Color::Yellow)))?;
    writeln!(stdout, " ({})", crate::util::format_long_datetime(&message.date.0))?;
    stdout.reset()?;
    writeln!(stdout, "{}\n", message.message)?;
  }
...
fn get_change_id_from_git() -> Result<String, failure::Error> {
  ...
  let message = commit
    .message()
    .ok_or(failure::err_msg("Could not discover change-id: HEAD commit message empty"))?;
