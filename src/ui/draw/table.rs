...
use crate::ui::table::{Column, ColumnBuiltIn, ColumnValue, Columns, Row, Selection, Table, VerticalScroll};
...
pub fn draw_table<W>(
  stdout: &mut W,
  (rect, table, columns, vscroll, selection, winbox): (&Rect, &Table, &Columns, Option<&VerticalScroll>, Option<&Selection>, Option<&WinBox>),
) -> Option<()>
where
  W: std::io::Write,
{
  ...
  if columns.print_header {
    ...
    queue!(stdout, cursor::MoveDown(1), cursor::MoveToColumn(rect.x.0 + 1 /*begins on one*/)).unwrap();
...
where
  W: std::io::Write,
{
  let draw_column_header = &mut |column: &Column, column_separator: &str, available_column_width: usize| {
    let actual_column_name = super::format_strip_align(&column.name, available_column_width, &column.alignment);
    queue!(
      stdout,
      style::PrintStyledContent(StyledContent::new(ContentStyle::default(), &column_separator,)),
      style::PrintStyledContent(StyledContent::new(column.style.clone(), &actual_column_name,))
    )
    .unwrap();
  };
...
fn draw_table_rows<W>(
  stdout: &mut W, (rect, table, columns, vscroll, selection): (&Rect, &Table, &Columns, Option<&VerticalScroll>, Option<&Selection>),
) where
  W: std::io::Write,
{
  let draw_row = |_iter_idx: usize, row_idx: usize, row: &Row| {
    ...
    let draw_cell = |column: &Column, column_separator: &str, available_column_width: usize| {
      ...
      let actual_content = super::format_strip_align(&content, available_column_width, &column.alignment);
      let actual_style = row_style.or(style).unwrap_or_default();
      queue!(
        stdout,
        style::PrintStyledContent(StyledContent::new(actual_style.clone(), &column_separator,)),
        style::PrintStyledContent(StyledContent::new(actual_style.clone(), &actual_content,))
      )
      .unwrap();
    };
...
///     row: row object
fn foreach_visible_row_compute_and_draw<F>((rect, table, vscroll): (&Rect, &Table, Option<&VerticalScroll>), mut draw_callback: F)
where
...
fn foreach_column_compute_width_and_draw<F>((rect, columns): (&Rect, &Columns), mut draw_callback: F)
where
...
fn resolve_column_content(row_idx: usize, row: &Row, column_value: &ColumnValue, selection: Option<&Selection>) -> (String, Option<ContentStyle>) {
  match column_value {
    ColumnValue::BuiltIn { builtin } => match builtin {
      ColumnBuiltIn::LineNumber { mode, style } => {
        let number = resolve_column_builtin_line_number(mode, row_idx, selection).to_string();
...
fn resolve_column_builtin_line_number(mode: &LineNumberMode, row_idx: usize, selection: Option<&Selection>) -> usize {
  match mode {
...
mod test {
  ...
  use crate::ui::table::{resolve_line_number_column_width, Column, ColumnIndex, ColumnValue, Row};
  ...
  fn table_components() -> (Table, Columns) {
    ...
    let table = Table { rows: vec![row1, row2] };
    let columns = Columns {
      print_header: true,
      visible: vec![
        Column {
  ...
  fn multiple_columns_high_width() {
    let expected = vec!["  |commit  |number     ", " 1|8f524ac |104508     ", " 2|18d3290 |104525     ", ""];
  ...
  fn no_table_entries_to_print() {
    ...
    let table = Table { rows: Vec::default() };
    let mut output: Vec<u8> = Vec::new();
    draw_table(&mut output, (&rect, &table, &columns, None, None, None));
    let output = strip_ansi_escapes(output);
    assert_eq!(expected, output);
  ...
  fn select_first_row() {
    ...
    let mut output: Vec<u8> = Vec::new();
    draw_table(&mut output, (&rect, &table, &columns, None, Some(&selection), None));
  ...
  fn select_second_row() {
    ...
    row3.insert(ChangeColumn::Owner as ColumnIndex, String::from("Thomas Edison"));
  ...
  fn scroll_zero() {
    ...
    let mut output: Vec<u8> = Vec::new();
    draw_table(&mut output, (&rect, &table, &columns, Some(&vscroll), None, None));
    let actual_output = strip_ansi_escapes(output.clone());
    assert_eq!(expected_output, actual_output);
  }
  ...
  fn selection_plus_scroll() {
    ...
    let mut output: Vec<u8> = Vec::new();
    draw_table(&mut output, (&rect, &table, &columns, Some(&vscroll), Some(&selection), None));
    let actual_output = strip_ansi_escapes(output.clone());
    let actual_selection = get_reversed_rows(output.clone());
    assert_eq!(expected_output, actual_output);
    assert_eq!(expected_selection, actual_selection);
