...
use crate::ui::table::{
  resolve_line_number_column_width, Column, ColumnBuiltIn, ColumnIndex, ColumnValue, Columns, Row, Selection, Table, VerticalScroll,
};
...
pub fn create_table((width, height): (TermUSize, TermUSize), registry: &mut World) {
  ...
  row.insert(ChangeColumn::Subject as ColumnIndex, String::from("Remove Conditional verification info"));
  ...
  let mut table = Table { rows: vec![row, row2, row3] };
  ...
  let columns = Columns {
    ...
    visible: vec![
      ...
      Column {
        ...
        width: 7,
        style: ContentStyle::new().attribute(Attribute::Bold).attribute(Attribute::Underlined),
  ...
  let winbox = WinBox {
    ...
    bottom_hints: vec![
      BoxHint {
        ...
        style: ContentStyle::new().attribute(Attribute::Dim).attribute(Attribute::Reverse),
        margin: HorizontalMargin { left: 1, right: 0 },
        alignment: HorizontalAlignment::Left,
      },
      BoxHint {
